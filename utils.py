...
def count_degree(adj):
    ...
    sorted_degrees_indices = np.argsort(degrees)    # 根据度数进行排序
    top_threshold = np.percentile(degrees, 99)    # 计算前1%大的度的阈值
    top_indices = sorted_degrees_indices[degrees[sorted_degrees_indices] >= top_threshold]    # 获取前1%大的度的节点索引
    top_degrees = degrees[top_indices]      # 获取前1%大的度的数目
    return top_indices, top_degrees

def tensor_to_sparse(tensor, shape_num):       # 将tensor转化为稀疏矩阵
    rows = tensor[0].cpu().numpy()  # 起始节点索引
    cols = tensor[1].cpu().numpy()  # 结束节点索引
    data = np.ones(rows.shape, dtype=np.int8)
    sparse_matrix = coo_matrix((data, (rows, cols)), shape=(shape_num, shape_num), dtype=np.int8)
...
def noisify(y, p_minus, p_plus=None, random_state=0):
    ...
    改变标签，概率为p_minus，如果p_plus提供就用p_plus提供的概率
    """
...
def load_emd(path, dataset):
    # 从文件中加载数据集的嵌入向量，并将其存储到 NumPy 数组中以便后续操作
